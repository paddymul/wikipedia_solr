...
Note: The "Rows Fetched" will get to 12 million, and then the system
will take aproximately 1 more hour compacting and optimizing indexes
...
To get around this, you can symlink the existing dump file into a
local directory.  Symlink just creates a pointer so that other
applications think the file they are pointing to is in the local
directory when in actuality it is in the foreign directory.

This fix requires 500-3000Mb free on the local disk.

execute the following in a terminal window.  They will create a new
directory off of your home directory named "new_dl_dir", that
directory will become the new GC_WIKIPEDIA_DL_DIR .
#+BEGIN_SRC shell
source ~/.profile
export FULL_BZ2=$GC_WIKIPEDIA_DL_DIR/enwiki-latest-pages-articles.xml.bz2

cd ~

mkdir new_dl_dir
cd new_dl_dir
ln -s $FULL_BZ2 ./enwiki-latest-pages-articles.xml.bz2
echo "export GC_WIKIPEDIA_DL_DIR=\"`pwd`\" " >> ~/.profile
#+END_SRC

*** I have already downloaded the wikipedia but I want to run makehelper files
Sometimes you will want to rerun the makehelper scripts that create
